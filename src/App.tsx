...
const OPENROUTER_STORAGE_KEY = 'whitechat_openrouter_api_key';
...
function App() {
  ...
  const handleManageApiKey = () => {
    ...
    const message = openRouterApiKey
      ? 'Update your OpenRouter API key. Leave blank to remove the saved key.'
      : 'Add your OpenRouter API key. You can generate one from https://openrouter.ai/keys.';
    const nextKey = window.prompt(message, openRouterApiKey);
  ...
  const handleSendMessage = async (event?: FormEvent<HTMLFormElement>) => {
    ...
    if (!apiKey) {
      setChatError(
        'Add your OpenRouter API key from the chat header to send messages. Keys can be created at https://openrouter.ai/keys.',
      );
      return;
    }
      ...
      <main className="chat">
        <header className="chat-header">
          ...
          <div className="chat-header-actions">
            <button type="button" className="api-key-button" onClick={handleManageApiKey}>
              {openRouterApiKey ? 'Update API key' : 'Add API key'}
            </button>
            <span className="api-key-status">
              {openRouterApiKey ? 'Stored securely on this device' : 'Required before sending messages'}
            </span>
          </div>
        </header>
